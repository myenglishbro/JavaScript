...
router.get("/insertion",async (req,res)=>{
    let result= await estudianteModel.insertMany(estudiantes)
    res.send({result})
})


router.get("/",async (req,res)=>{
    let result= await estudianteModel.find()
...
router.put("/:sid",async(req,res)=>{
    ...
    const updateEstudiantes=req.body;
    const result=await estudianteModel.updateOne({_id:id},{$set:updateEstudiantes});
    res.send({result})
})
